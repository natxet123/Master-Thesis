...
for j in np.arange(21,22,7):     # for j in (7,14,21)  # [7,14,21]: #
    # Option for selecting data
    UTrain = Utrain[:, 0:j]    #7-14-21 number of inputs
    yTrain = cytrain

    UTest = Utest[:, 0:j]       #7-14-21
    yTest = cytest

    UDev = Udev[:, 0:j]        #7-14-21
    yDev = cydev
    ...
    features = j    #number of column inputs (probes)
    ...
    model.save('FLSTM_Best_DR' + str(j) , save_format='h5')
    hist = pd.DataFrame(runfit.history)    # progress printen
    hist.to_csv('FLSTM_Best_DR' + str(j) + '_hist.csv')

    '''# Load model
    else:
        model = K.models.load_model('LSTM_Best_DR' + str(j) )'''
    ...
    for row in open('FLSTM_Best_DR' + str(j) + '_hist.csv'):
      rowcount+= 1
     #printing the result
    print(rowcount)
    ##########################################
    ...
    with open('LSTM_Best_DR_Results.csv', 'a') as f_object:
    ...
    ModelPerfo.index.name = 'FLSTM_Best_DR' + str(j) + '_perfo'
    ModelPerfo.to_csv('FLSTM_Best_DR' + str(j) + '_perfo.csv', header=['Value'])

    '''# Plots
    hist = pd.read_csv('LSTM_Best_DR' + str(j) + '_hist.csv')
