...
neurons = np.arange(5, 51, 5)    #vector: starting from 5 til 50 each 5
...
for j in neurons:
    ...
    model.add(Dense(j, activation='relu', name='layer2'))
    ...
    model.save('LSTM_' + '2layers_' + str(j) + 'neurons', save_format='h5')
    hist = pd.DataFrame(runfit.history)    # progress printen
    hist.to_csv('LSTM_' + '2layers_' + str(j) + 'neurons' + '_hist.csv')

    '''# Load model
    else:
        model = K.models.load_model('LSTM_' + '2layers_' + str(j) + 'neurons')'''
    ...
    for row in open('LSTM_' + '2layers_' + str(j) + 'neurons' + '_hist.csv'):
      rowcount+= 1
     #printing the result
    print(rowcount)
    ##########################################
    ...
    with open('LSTM_Neuron_LoopResults.csv', 'a') as f_object:
    ...
    ModelPerfo.index.name = 'LSTM_' + '2layers_' + str(j) + 'neurons' + '_perfo'
    ModelPerfo.to_csv('LSTM_' + '2layers_' + str(j) + 'neurons' + '_perfo.csv', header=['Value'])
    #ModelPerfo.to_latex('LSTMM_f_50ts' + str(name) + '_perfo.tex', header=['Value'])
    
    '''# Plots
    hist = pd.read_csv('LSTM_' + '2layers_' + str(j) + 'neurons' + '_hist.csv')
