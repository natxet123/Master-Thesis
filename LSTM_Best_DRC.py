...
UTrain = Utrain[:,[0,7,14,1,8,15,2,9,16,3,10,17,4,11,18,5,12,19,6,13,20]] #0,7,14,1,8,15,2,9,16,3,10,17,4,11,18,5,12,19,6,13,20
yTrain = cytrain

# PLOT Utrain

UTest = Utest[:, [0,7,14,1,8,15,2,9,16,3,10,17,4,11,18,5,12,19,6,13,20]]
yTest = cytest

UDev = Udev[:, [0,7,14,1,8,15,2,9,16,3,10,17,4,11,18,5,12,19,6,13,20]]
yDev = cydev
...
features = 3*j    #number of column inputs (probes)
...
UTrain_LSTM = np.zeros((batch_sizef, time_steps, features))     #see page 37
#print(UTrain_LSTM)
...
model.add(Dense(30, activation='relu', name='layer2'))        #2layers
...
model.save('FLSTM_Best_DRC_' + str(j) , save_format='h5')
hist = pd.DataFrame(runfit.history)    # progress printen
hist.to_csv('FLSTM_Best_DRC' + str(j) + '_hist.csv')

'''# Load model
else:
    model = K.models.load_model('LSTM_Best_DRC' + str(j) )'''
...
for row in open('FLSTM_Best_DRC' + str(j) + '_hist.csv'):
  rowcount+= 1
 #printing the result
print(rowcount)
##########################################
data = [j,rel_RMSE,rel_RMSEdev,rowcount,training_time]
print(data)
from csv import writer
with open('LSTM_Best_DRC_Results.csv', 'a') as f_object:
...
ModelPerfo.index.name = 'FLSTM_Best_DRC' + str(j) + '_perfo'
ModelPerfo.to_csv('FLSTM_Best_DRC' + str(j) + '_perfo.csv', header=['Value'])

'''# Plots
hist = pd.read_csv('LSTM_Best_DRC' + str(j) + '_hist.csv')
...
timec = timel[time_steps-1:]  # np.arange(0.00, len(yTest_LSTMc)*0.02, step=0.02)
timecutoff = timel[-20:]
