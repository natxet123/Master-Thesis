...
nm = 1  #for ifs cycles, if data already simulated use nm=0.
#name = '1Layer'      # 0.131 | 0.042
layers = 10
...
time_steps = 50  # let state evolve over 50 ts
batch_sizef = UTrain.shape[0]-time_steps+1  # -10 for future prediction     #.shape returns shape of array; .shape[0] returns dimensions for rows
#LSTM batch size: from u(1) to u(N-m+1)  (see page 37)
features = 21
LSTM_neurons = 40  # use 30 outputs which are recombined using a dense net afterwords     #LSTM neurons
...
yTrain_LSTM = yTrain[time_steps-1:]     # +10 for future prediction  # and same for development and testing data
...
yDev_LSTM = yDev[time_steps_dev-1:] #+10 for future prdiction
...
yTest_LSTM = yTest[time_steps_test-1:]  #1st output: row 49 (0+50-1) is 1st output (hm)
...
if nm:
    ...
    model.add(Dense(20, activation='relu', name='layer2'))        #2layers
    model.add(Dense(20, activation='relu', name='layer3'))         #3layers
    model.add(Dense(20, activation='relu', name='layer4'))         #4layers
    model.add(Dense(20, activation='relu', name='layer5'))         #5layers
    model.add(Dense(20, activation='relu', name='layer6'))          #6layers
    model.add(Dense(20, activation='relu', name='layer7'))          #7layers
    model.add(Dense(20, activation='relu', name='layer8'))          #8layers
    model.add(Dense(20, activation='relu', name='layer9'))          #9layers
    model.add(Dense(20, activation='relu', name='layer10'))          #10layers
    model.add(Dense(1, activation='linear', name='outputLayer'))    #1layer
  ...
  #AUTOMATE IT FOR NAMES
    model.save('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers', save_format='h5')
    hist = pd.DataFrame(runfit.history)    # progress printen
    hist.to_csv('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers' + '_hist.csv')

# Load model
else:
    model = K.models.load_model('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers')
...
##########################################
#with open('LSTM_' + str(time_steps) +'ts_' + str(name) + '_hist.csv') as f:
    #sum(1 for line in f)
epochs = sum(1 for row in ('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers' + '_hist.csv'))
print(epochs)

#Setting initial value of the counter to zero
rowcount = -1
#iterating through the whole file
for row in open('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers' + '_hist.csv'):
  rowcount+= 1
 #printing the result
print(rowcount)
##########################################
data = [layers,rel_RMSEdev,rowcount,training_time]
print(data)
from csv import writer
with open('LSTM_RMSE_layers2.csv', 'a') as f_object:
...
if nm:  #TABLE
    ...
    ModelPerfo.index.name = 'LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers' + '_perfo'
    ModelPerfo.to_csv('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers' + '_perfo.csv', header=['Value'])
    #ModelPerfo.to_latex('LSTMM_f_50ts' + str(name) + '_perfo.tex', header=['Value'])

# Plots
hist = pd.read_csv('LSTM_' + str(time_steps) +'ts_' + str(layers) + 'layers' + '_hist.csv')
...
timec = timel[time_steps-1:]  # np.arange(0.00, len(yTest_LSTMc)*0.02, step=0.02)     #timestep is gekend als 0.01, tijdsrange namaken | hier ineens wel 0 als beginwaarde gebruiken?
timecutoff = timel[-20:]
