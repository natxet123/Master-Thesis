...
time_steps = np.arange(20, 161, 20)    #vector: starting from 20 til 160 each 20

for j in time_steps:
    batch_sizef = UTrain.shape[0] - j + 1  #.shape returns shape of array; .shape[0] returns dimensions for rows
    ...
    UTrain_LSTM = np.zeros((batch_sizef, j, features))
    # print(UTrain_LSTM)

    for i in range(batch_sizef):
        UTrain_LSTM[i, :, :] = UTrain[i:i + j, :]  # [0-9],[1-10],[2-11],...
    ...
    yTrain_LSTM = yTrain[j - 1:]  # +10 for future prediction  # and same for development and testing data
    # Cut off first time steps of output so that output matches last time step of input
    # Idem for validation data
    time_steps_dev = j  # must be equal!
    ...
    # Testing data
    time_steps_test = j  # must be equal!
    ...
    model = K.Sequential()
    model.add(LSTM(LSTM_neurons, input_shape=(j, features),
    ...
    model.save('LSTM_' + str(j) + 'ts', save_format='h5')
    hist = pd.DataFrame(runfit.history)  # progress printen
    hist.to_csv('LSTM_' + str(j) + 'ts' + '_hist.csv')

    '''#  Load model
    else:
        model = K.models.load_model('LSTM_' + str(j) +'ts')'''
    ...
    # sum(1 for line in f)
    epochs = sum(1 for row in ('LSTM_' + str(j) + 'ts' + '_hist.csv'))
    print(epochs)

    # Setting initial value of the counter to zero
    rowcount = -1
    # iterating through the whole file
    for row in open('LSTM_' + str(j) + 'ts' + '_hist.csv'):
        rowcount += 1
    # printing the result
    print(rowcount)
    ##########################################
    ...
    with open('LSTM_TS_LoopResults.csv', 'a') as f_object:
    ...
    ModelPerfo.index.name = 'LSTM_' + str(j) + 'ts' + '_perfo'
    ModelPerfo.to_csv('LSTM_' + str(j) + 'ts' + '_perfo.csv', header=['Value'])
    '''
    # Plots
    hist = pd.read_csv('LSTM_' + str(j) + 'ts' + '_hist.csv')
    ...
    timec = timel[j - 1:]  # np.arange(0.00, len(yTest_LSTMc)*0.02, step=0.02)     #timestep is gekend als 0.01, tijdsrange namaken | hier ineens wel 0 als beginwaarde gebruiken?
    timecutoff = timel[-20:]
