...
for i in range(1,6):
    mv = mv + 1
    TestModel = K.Sequential([Dense(neuron1, activation='relu', input_shape=[len(Utrain[1]), ], name='hidden1'),
                              Dense(neuron2, activation='relu', name='Hidden2'),
                              Dense(neuron3, activation='relu', name='Hidden3'),
                              ...
                              Dense(1, activation='linear', name='output')], name=str(nl)+'hl')
    ...
    hist.to_csv('Best_Model' + str(mv) +'_History.csv')      # save under a different number each time so you don't overwrite your previous models, they might have been better (plot this to evaluate training process)
    TestModel.save('Best_Model' + str(mv), save_format='h5')
    '''else:
        TestModel = K.models.load_model('Best_Model' + str(mv))
        #TestModel.summary()'''
    ...
    data = [mv,rel_RMSE,rel_RMSEdev]
    from csv import writer
    with open('RMSE_best.csv', 'a') as f_object:
    ...
    Model1Perfo.to_csv('Best_Model' + str(mv) + '_perfo.csv', header=['Value'])
