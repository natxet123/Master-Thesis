...
for i in range (1,6):
    ...
    model.save('FLSTM_Best_' + str(mv) , save_format='h5')
    hist = pd.DataFrame(runfit.history)    # progress printen
    hist.to_csv('FLSTM_Best_' + str(mv) + '_hist.csv')

    '''# Load model
    else:
        model = K.models.load_model('LSTM_Best_' + str(mv))'''
    ...
    for row in open('FLSTM_Best_' + str(mv) + '_hist.csv'):
      rowcount+= 1
     #printing the result
    print(rowcount)
    ##########################################
    data = [mv,rel_RMSE,rel_RMSEdev,rowcount,training_time]
    print(data)
    from csv import writer
    with open('LSTM_Best_Results.csv', 'a') as f_object:
    ...
    ModelPerfo.index.name = 'FLSTM_Best_' + str(mv) + '_perfo'
    ModelPerfo.to_csv('FLSTM_Best_' + str(mv) + '_perfo.csv', header=['Value'])
    '''
    # Plots
    hist = pd.read_csv('LSTM_Best_' + str(mv) + '_hist.csv')
