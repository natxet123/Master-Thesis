...
features = 21
LSTM_neurons = np.arange(10, 81, 10)
...
for j in LSTM_neurons:
    ...
    model = K.Sequential()
    model.add(LSTM(j, input_shape=(time_steps, features),
    ...
    model.save('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons', save_format='h5')
    hist = pd.DataFrame(runfit.history)    # progress printen
    hist.to_csv('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons' + '_hist.csv')

    '''# Load model
    else:
        model = K.models.load_model('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons')'''
    ...
    #with open('LSTM_' + str(time_steps) +'ts_' + str(name) + '_hist.csv') as f:
        #sum(1 for line in f)
    epochs = sum(1 for row in ('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons' + '_hist.csv'))
    print(epochs)

    #Setting initial value of the counter to zero
    rowcount = -1
    #iterating through the whole file
    for row in open('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons' + '_hist.csv'):
      rowcount+= 1
     #printing the result
    print(rowcount)
    ##########################################
    data = [j,rel_RMSEdev,rowcount,training_time]
    print(data)
    from csv import writer
    with open('LSTM_LSTMneuron_LoopResults.csv', 'a') as f_object:
    ...
    ModelPerfo.index.name = 'LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons' + '_perfo'
    ModelPerfo.to_csv('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons' + '_perfo.csv', header=['Value'])
    #ModelPerfo.to_latex('LSTMM_f_50ts' + str(name) + '_perfo.tex', header=['Value'])

    '''# Plots
    hist = pd.read_csv('LSTM_' + str(time_steps) +'ts_' + str(j) + 'LSTMneurons' + '_hist.csv')
